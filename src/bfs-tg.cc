...
int64_t
BUStep_rec(const Graph &g, pvector<NodeID> &parent, Bitmap &front,
		Bitmap &next, NodeID start, NodeID end) {
	int64_t awake_count = 0;
	debug("called BU with " << start << ".." << end);
	if (end - start <= 1) {
		...
		NodeID cut = (end - start) / 2;
		int64_t x, y;
		oneapi::tbb::task_group tg;
		tg.run([=, &g, &parent, &front, &next, &x] {
			x = BUStep_rec(g, parent, front, next, start,
					start + cut);
		});
		tg.run([=, &g, &parent, &front, &next, &y] {
			y = BUStep_rec(g, parent, front, next, start + cut + 1,
					end);
		});
		tg.wait();
		awake_count = x + y;
	}
	debug("awake_count = " << awake_count);
	return awake_count;
...
int64_t
TDStep_rec(const Graph &g, pvector<NodeID> &parent,
		SlidingQueue<NodeID> &queue, NodeID *start, NodeID *end)
{
	int64_t scout_count = 0;
	debug("called TD with " << start << ".." << end);
	if (end - start <= 1) {
		QueueBuffer<NodeID> lqueue(queue);
		for (NodeID *q_iter = start; q_iter <= end; ++q_iter) {
			NodeID u = *q_iter;
			for (NodeID v : g.out_neigh(u)) {
				NodeID curr_val = parent[v];
				if (curr_val < 0) {
					if (compare_and_swap(parent[v],
							curr_val, u)) {
						lqueue.push_back(v);
		...
		int64_t x, y;
		oneapi::tbb::task_group tg;
		tg.run([=, &g, &parent, &queue, &x] {
			x = TDStep_rec(g, parent, queue, start, start + cut);
		});
		tg.run([=, &g, &parent, &queue, &y] {
			y = TDStep_rec(g, parent, queue, start + cut + 1, end);
		});
		tg.wait();
		scout_count = x + y;
	}
	debug("scout_count =  " << scout_count);
	return scout_count;
}
  ...
  while (!queue.empty()) {
    if (scout_count > edges_to_check / alpha) {
      ...
      do {
        ...
        curr.reset();
        awake_count = BUStep_rec(g, parent, front, curr, 0, g.num_nodes() - 1);
        front.swap(curr);
        t.Stop();
      ...
      scout_count = TDStep_rec(g, parent, queue, queue.begin(), queue.end()-1);
      queue.slide_window();
      t.Stop();
